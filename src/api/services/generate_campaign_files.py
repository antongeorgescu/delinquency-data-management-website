...
Usage:
    python generate_campaign_files.py
...
def generate_campaign_summary_json(df, risk_label):
    """
    Generate summary statistics for campaign data and return as dictionary.
    """
    if len(df) == 0:
        return {
            "total_borrowers": 0,
            "message": f"No {risk_label} borrowers found"
        }
    
    summary = {
        "total_borrowers": len(df),
        "average_age": round(df['age'].mean(), 1),
        "average_income": int(df['annual_income_cad'].mean()),
        "average_loan_amount": int(df['loan_amount'].mean()),
        "average_monthly_payment": int(df['monthly_payment'].mean()),
        "geographic_distribution": {},
        "program_difficulty_distribution": {},
        "income_categories": {}
    }
    ...
    for province, count in province_dist.items():
        ...
        summary["geographic_distribution"][province] = {
            "count": int(count),
            "percentage": pct
        }
    ...
    if 'program_difficulty_label' in df.columns:
        ...
        for difficulty, count in difficulty_dist.items():
            pct = round((count / len(df)) * 100, 1)
            summary["program_difficulty_distribution"][difficulty] = {
                "count": int(count),
                "percentage": pct
            }
        ...
        for difficulty, count in difficulty_dist.items():
            difficulty_label = f"Difficulty Level {difficulty}"
            pct = round((count / len(df)) * 100, 1)
            summary["program_difficulty_distribution"][difficulty_label] = {
                "count": int(count),
                "percentage": pct
            }
    ...
    for category, count in income_dist.items():
        pct = round((count / len(df)) * 100, 1)
        summary["income_categories"][category] = {
            "count": int(count),
            "percentage": pct
        }
...
def generate_campaign_summary(df, risk_label):
    """
    Generate summary statistics for campaign data (original function with print statements).
    """
    if len(df) == 0:
...
def generate_campaign_files():
    ...
    try:
        ...
        if total_with_risk == 0:
            ...
            print("Please run 'python run_risk_estimation.py' first to calculate risk scores.")
            return False
...
Each file contains comprehensive borrower information for targeted campaigns.
    ...
    parser.add_argument(
        "--output-dir", 
        default="campaigns",
        help="Output directory for campaign files (default: campaigns)"
    )
    
    args = parser.parse_args()
    
    try:
        # Run campaign files generation
        success = generate_campaign_files()
        
        if success:
            print(f"\n✅ Campaign files generation completed successfully!")
            sys.exit(0)
        else:
            print(f"\n❌ Campaign files generation failed!")
            sys.exit(1)
            
    except KeyboardInterrupt:
        print(f"\n⚠️ Campaign generation interrupted by user")
        sys.exit(1)
        
    except Exception as e:
        print(f"\n❌ Error during campaign generation: {str(e)}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
def generate_campaign_files_json():
    """
    Generate campaign files and return results as JSON-compatible dictionary.
    
    Returns:
        dict: JSON-compatible results with file URLs and analysis data
    """
    results = {
        "success": False,
        "message": "",
    ...
    try:
        ...
        if total_with_risk == 0:
            results["message"] = "No delinquency risk scores found in database. Please run risk estimation first."
            return results
        ...
        for row in risk_dist_data:
            ...
            if risk_level is not None:
                label = risk_labels.get(risk_level, f"Risk {risk_level}")
                pct = round((count / total_with_risk) * 100, 1)
                results["risk_distribution"][str(risk_level)] = {
                    "label": label,
                    "count": int(count),
                    "percentage": pct
                }
        ...
        for risk_level, risk_label, filename in campaign_configs:
            ...
            # Generate summary
            campaign_summary = generate_campaign_summary_json(df, risk_label)
            results["campaign_summaries"][risk_label.lower().replace(" ", "_")] = campaign_summary
            ...
            if len(df) > 0:
                ...
                results["files_generated"].append({
                    ...
                    "url": f"/api/download-campaign-file/{filename}",
        ...
        # Update statistics
        results["statistics"] = {
            "total_borrowers_with_risk": int(total_with_risk),
            "files_generated_count": total_files_generated,
            "campaigns_directory": os.path.abspath(campaigns_dir)
        }
        
        results["success"] = True
        results["message"] = f"Campaign files generation completed successfully. Generated {total_files_generated} files."
        
        return results
        
    except Exception as e:
        results["message"] = f"Error during campaign generation: {str(e)}"
